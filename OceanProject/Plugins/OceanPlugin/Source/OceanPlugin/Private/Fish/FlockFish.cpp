...
AFlockFish::AFlockFish(const FObjectInitializer& ObjectInitializer) : Super(ObjectInitializer)
{
	...
	underwaterMin = FVector(fishFlockMin.X, fishFlockMin.Y, fishFlockMin.Z);
	underwaterMax = FVector(fishFlockMax.X, fishFlockMax.Y, fishFlockMax.Z);
	//underwaterMin = FVector(0, 0, 0);
	//underwaterMax = FVector(0, 0, 0);
	CustomZSeekMin = 0.0;
	CustomZSeekMax = 0.0;
...
void AFlockFish::MoveBounds(float delta)
{
	if (hasFishManager)
	{
		...
		*/
		FVector fishManagerPosition = fishManager->GetActorLocation();
		maxX = fishManagerPosition.X ;
		minX = fishManagerPosition.X ;
		maxY = fishManagerPosition.Y ;
		minY = fishManagerPosition.Y ; 
		/*
		FVector actorLocation = this->GetActorLocation();
...
{
	target = FVector(FMath::FRandRange(fishFlockMin.X, fishFlockMax.X), FMath::FRandRange(fishFlockMin.Y, fishFlockMax.Y), FMath::FRandRange(fishFlockMin.Z, fishFlockMax.Z));
}
/*
void AFlockFish::spawnTarget()
{
	target = FVector(-6888,-23004,-9375);
}*/
